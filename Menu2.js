...
class App extends React.PureComponent {
  ...
  getAllActions = id => {
    Axios.post(`${api}/cms/rest/action/getAllActions`, {
      sessionKey: id
    }).then(res => {
      return this.setState({ AllActions: res.data.resultList });
    });
  ...
  takeAction = id => {
    ...
    Axios.post(`${api}/cms/rest/task/takeAction`, bodyObj).then(res =>
  ...
  componentDidMount() {
    AsyncStorage.getItem("userData").then(token => {
      this.getAllActions(JSON.parse(token).session);
  ...
  render() {
    return (
      ...
      >
        <TouchableOpacity
          onPress={() => this.setState({ showModalOptions: true })}
          style={{
            zIndex: 999
          }}
        >
                      ...
                      {this.state.AllActions.map(action => {
                        return (
                          <TouchableOpacity
                            key={action.configId}
                            onPress={() => {
                              this.takeAction(action.configId);
                              this.setState({ showModalOptions: false });
                            }}
                          >
                            <Text
